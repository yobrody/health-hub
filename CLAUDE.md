...
## Open / pending
1. ⚠️ **BLOCKED — needs Brody:** set `SCAN_SAMPLES_TOKEN = hh_scan_431NLxeqrWM790HmkC3pB7qIKZP9RWWY` in Cloudflare Pages (Production) to activate the nightly scan self-improvement review reads. Tried to set it via the CF API on 2026-08-17 but the vault's **"API Token (Pages Edit)" is invalid/expired** (401 on `/user/tokens/verify`). Either set it in the CF dashboard or mint a fresh Pages-Edit token and drop it in the vault. No-ops harmlessly until set.
